...
for province_name, province_code in province_codes.items():
    try:
        ...
        if "running text" in df_hourly.columns:
            # Filter hanya data dengan nilai > 0
            df_hourly = df_hourly[df_hourly["running text"] > 0]
            
            if not df_hourly.empty:
                data_provinsi[province_name] = df_hourly
                
                # Simpan data ke CSV per provinsi
                csv_filename = f"tren_running_text_{province_name}.csv"
            ...
            print(f"Tidak ada data untuk keyword 'running text' di {province_name}")
...
if data_provinsi:
    ...
    for province_name, df_hourly in data_provinsi.items():
        plt.plot(df_hourly.index, df_hourly["running text"], label=province_name)
    
    plt.xlabel("Waktu (Per Jam)")
    plt.ylabel("Tren Pencarian")
    plt.title("Tren Pencarian 'running text' per Jam di Provinsi Pulau Jawa")
